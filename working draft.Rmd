...
```{r read data}
hosp <- fread("C:\\Users\\Rakib Kamal\\Box Sync\\Fall 2019\\DS4001\\Projects\\Final\\hospital.csv", 
              check.names=T,
              na.strings = c("", "NA")
)
...
# read in income file
incomes <- read.csv(file="C:\\Users\\Rakib Kamal\\Documents\\GitHub\\FinalProject\\zipcodes - Sheet1.csv")
colnames(incomes)[1] <- "Zip" # rename zip.code column
# perform left outer join on our hosp dataset and the income dataset, b y Zipcode
...
# read in county data
county <- read.csv("C:\\Users\\Rakib Kamal\\Documents\\GitHub\\FinalProject\\county.csv")
...
```{r}
set.seed(101)
index <- createDataPartition(hosp$Charges,p = 0.8, list = FALSE)
train <- hosp[index,]
test <- hosp[-index,]
```
...
features <- train %>% select("Charges", "Costs", "Duration", "Severity.Code")%>% filter(!is.na(Severity.Code))
fit <- randomForest(Charges~., data=features, 
                     replace=T, # changed replace=T instead of smaller sample size
                     ntree=500,
                     mtry=2, # mtry=number of features at each node (sqrt(11ish))
                     importance=T)
varImp(fit)
