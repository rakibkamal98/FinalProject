...
ggplot(hosp, aes(x=Age, y=Charges, fill=factor(Gender, levels=c("M", "F", "Unknown")))) +
  ...
  theme(legend.position=c(.12, .80), legend.background=element_rect(fill="white", color="grey"))
  xlab("Age Group of Patient") +
  ylab("Charges ($)")
```
...
Investigating the weird magic slope
```{r}
# in search of the magic slope
temp <- hosp %>%
...
ggplot(temp, aes(x=Charges, y=Costs)) +
  geom_point(alpha=.3)
# magic slope looks (by visual inspection) to be 2/3
# filter between .7 and .6 chargeratio
temptemp <- temp %>%
...
# filter between .675 and .65 chargeratio
...
table(filtered$Surgical) # all medical severity
table(filtered$Payment1) # almost all are Medicaid or Medicare
table(filtered$Facility.ID)
```
...
ggplot(hosp_temp, aes(x=as.factor(DRG.Code), y=Charges, fill=proc)) +
  ...
  theme(legend.position=c(.8,.88), 
        legend.background=element_rect(fill="white", color="grey"),
        legend.title="Type of Cardiac Procedure")
...
# create DRG vars based off preceding graphs
hosp <- hosp %>%
...
myindex <- sample(1:nrow(hosp), 100000)
mysamp <- hosp[myindex,]

write.csv(mysamp, file="sample for ml.csv", row.names=F)
```