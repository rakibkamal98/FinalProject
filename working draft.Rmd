...
# Test Center
```{r}
ggplot(hosp, aes(x=Type)) + geom_bar(stat="count") + theme_minimal()
ggplot(hosp, aes(x=Discharge.Status)) + geom_bar(stat="count") + theme_minimal() + theme(axis.text.x = element_text(angle=90))

ggplot(hosp, aes(x=CSS.Code)) + geom_histogram()
table(hosp$CSS.Code[hosp$CSS.Code > 260])
# categories above 650-63 and 670 represent MHSA coding (mental health codes)
# that leaves 917 and 999 as weird ones. 917 stands for unclassified E code and 999 is ungroupable. 
