...
# 2, 12, 13 have lower durations and fewer high outliers. 1, 17-19, and 22-25 have higher average durations. We also see this played out in their averages.
hosp %>%
  group_by(MDC.Code) %>%
  summarise(
    dur = mean(Duration),
...
ggplot(hosp, aes(x=chargeratio)) +
  ...
  xlim(0, 10) +
  geom_vline(xintercept=median(hosp$chargeratio), linetype="dashed")
